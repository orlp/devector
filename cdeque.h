...
/*
    ...
    Valid moved-from state: impl.clear().
*/
...
public:
    ...
    cdeque() noexcept(std::is_nothrow_default_constructible<Allocator>::value) : impl() {
        impl.clear();
    }

    explicit cdeque(const Allocator& alloc) noexcept : impl(alloc) { impl.clear(); }

    explicit cdeque(size_type n, const Allocator& alloc = Allocator()) : impl(alloc) {
        impl.begin_storage = impl.begin_cursor = alloc_traits::allocate(impl, n);
        impl.end_storage = impl.end_cursor = impl.begin_storage + n;
        uninitialized_fill(impl.begin_cursor, impl.end_cursor);
    }

    cdeque(size_type n, const T& value, const Allocator& alloc = Allocator()) : impl(alloc) {
        impl.begin_storage = impl.begin_cursor = alloc_traits::allocate(impl, n);
        impl.end_storage = impl.end_cursor = impl.begin_storage + n;
        uninitialized_fill(impl.begin_cursor, impl.end_cursor, value);
    }

    template<class InputIterator>
    cdeque(InputIterator first, InputIterator last, const Allocator& alloc = Allocator())
    : impl(alloc) {
    ...
    cdeque(const cdeque<T, Allocator>& other, const Allocator& alloc) : impl(alloc) {
        init_range(other.begin(), other.end(), std::random_access_iterator_tag());
    }

    cdeque(cdeque<T, Allocator>&& other) noexcept : impl(std::move(other.impl)) {
    ...
    cdeque(cdeque<T, Allocator>&& other, const Allocator& alloc) : impl(alloc) {
    ...
    cdeque(std::initializer_list<T> il, const Allocator& alloc = Allocator()) : impl(alloc) {
    ...
    cdeque<T, Allocator>& operator=(std::initializer_list<T> il) { assign(il); return *this; }
    ...
    void reserve_front(size_type n) {
        if (n > max_size()) throw std::length_error("cdeque");
        if (capacity() < n) {
            pointer new_storage;
            try { new_storage = alloc_traits::allocate(impl, n); } catch(...) { return; }

            // TODO: strong exception guarantee
            unitialized_copy(std::move_iterator<iterator>(begin()),
    ...
    reference         at(size_type i) {
        if (i >= size()) throw std::out_of_range("cdeque");
        return (*this)[i];
    }
...
private:
    ...
    // Deletes all elements and deallocates memory. Does not leave the cdeque in a valid state!
    void destruct() {
        clear();
    ...
    // Initializes the cdeque with copies from [first, last). Strong exception guarantee.
    template<class InputIterator>
